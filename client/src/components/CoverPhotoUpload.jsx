    ...
    if (image) {
      ...
      try {
        const response = await axios.post(
          `${API_URL}/user/uploads/cover-photo`,
          formData,
          {
            headers: {
        ...
        // close popup
        setOpenUploadCoverPhoto(false);
        setCoverPhoto(response.data.updatedUser.cover_photo);

        toastHandler(
          <div className="flex gap-2 items-center">
            <CircleCheck className="bg-green-600 rounded-full text-white dark:text-[#242526]" />
            <span>Cover photo updated</span>
          </div>,
          false
        );
        setIsLoading(false);
